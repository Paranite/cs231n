...
def softmax_loss_naive(W, X, y, reg):
    ...
    for i in range(num_train):
        ...
        prob[0,y[i]] -= 1
        dW += np.dot(X[i].reshape(-1,1), prob) # or make another loop, going through dW[:,j] instead of reshaping "prob" and "X"
...
def softmax_loss_vectorized(W, X, y, reg):
    ...
    scores = X.dot(W)
    scores -= np.max(scores, axis = 1, keepdims = True) # By documentation this is equal to:
                                                        # scores -= np.amax(scores,axis = 1, keepdims = True)
                                                        # refer to : np.ndarray.max
